...
def pit2(x: float, x2: float, data: Tensor) -> float:
    return pit(x=x, data=data)




def one_x(x: Tensor, data: torch.Tensor) -> Tensor:
    indices = torch.tensor(range(num_feats)).to(dev)
    x_non_nan = torch.index_select(input=x, dim=0, index=indices)

    pit3 = partial(pit2, data=cdf_data)

    temp = torch.vmap(pit3)(indices, indices)
    return temp
