...
cdf_data: Tensor = torch.rand(size=(1000, 20)).to(dev)

class Vectorize1D():
    def __init__(self, func: Callable[[torch.Any, torch.int64], torch.Any], dev: device) -> None:
        self.func = torch.vmap(func=func)
        self.dev = dev
    

    def __call__(self, oned_tensor: Tensor) -> Tensor:
        size = oned_tensor.shape
        assert len(size) == 1, 'Only 1-D Tensors are supported.'
        indices = torch.Tensor(range(size[0])).type(torch.int64).to(self.dev)
        return self.func(oned_tensor, indices)


def my_func(val: torch.Any, idx: Tensor) -> Tensor:
    #return cdf_data[:,idx]
    return torch.select(input=cdf_data, dim=1, index=idx.int()[0])
    temp = torch.index_select(input=cdf_data, dim=1, index=idx)
    return temp


my_func_v = Vectorize1D(func=my_func, dev=dev)

temp = my_func_v(torch.tensor([6,7,8,9])).to(device=dev)
...
x: Tensor = torch.rand(size=(10, 20)).to(dev)
cdf_data: Tensor = torch.rand(size=(1000, 20)).to(dev)

def one_x(x: Tensor, data: torch.Tensor) -> Tensor:
    x_non_nan: Tensor = empty(size=(10,))
    #x_non_nan[0:10] = x[0:10]
    #indices = torch.linspace(start=20, end=20, steps=20, dtype=torch.int64).to(dev)
    indices = torch.tensor(range(20)).to(dev)
    x_non_nan = torch.index_select(input=x, dim=0, index=indices)
...
vec = torch.vmap(func=one_x, in_dims=0, out_dims=0)
